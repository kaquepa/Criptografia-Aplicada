...
def _decipher(file, method):
    with open(file) as f:
        ...
        #print('Ciphertext: ' , cyphertext[0].upper(), "\n" )
        ...
        if method == 'kasiski':
            ...
            key_len = find_key_length(cyphertext=cyphertext[0].upper(), seq_len=SEQ_LEN, max_key_len=MAX_KEY_LENGTH)
        ...
        for i in range(len(key_len)):
           key = key_refactor(cyphertext[0].upper(), key_len[i])
           decyphered = decypher(cyphertext[0].upper(), key)

           #print results of decryption 
           print('Chosen key length: '+str(key_len[i]))
